...
<div class="row">
	...
	<div>
		...
		<?php if($photoKey != ""): ?>
		<img class="img-fluid" src="img/img_users/<?php echo $photoKey ?>.jpg"  style=" width: 100%; height: 100%;" >
		<?php endif; ?>
		<?php if($photoKey == ""): ?>
		<img class="img-fluid" src="img/profil_male.jpg" height="460px" width="445px">
		<?php endif;?>
		<form method="POST" enctype="multipart/form-data" action="">
			<label>Insérez une photo de profil</label>
			<input type="file"  name="photo">
			<input type="submit" value ="Upload" name="upload_photo">envoyer</button>
		...
		if(isset($_POST['upload_photo'])){
			...
			$fileNameCmps = explode(".", $fileName);//On s'arrete de lire dès le point 
			$fileExtension = strtolower(end($fileNameCmps));//On continue de lire le nom après le point pour recup lextension
			$allowedfileExtensions = array('jpg', 'png');

			$newFileName = $controler->generateKey().'.'.$fileExtension; //On génère une clé de 8 carac
			$getKey = explode(".", $newFileName); 
			
			$string = implode('', $getKey);
			$string2 = substr($string,0,8); 
			//On recupère les info et on insert une clé à l'user
			
			$controler->insertImgKey($idfamille, $string2); 

			if(in_array($fileExtension, $allowedfileExtensions)){
				$location = "img/img_users/".$newFileName;
		...
		<div class="container" style="background-color: white">
			<?php foreach ($row as $rows): ?>
				<form>
					...
					<div class="form-group row">
						<label for="staticEmail" class="col-sm-2 col-form-label">Sexe : </label>
						 <div class="col-sm-5">
						 		<!-- On écrit le telephone -->
      						<input type="text" readonly class="form-control-plaintext" id="staticEmail" value="<?php echo $rows['sexe'] ?>">
    						</div>
					</div>
...
<div class="container-fluid" style="background-color: #e1d2b8;">
	...
	<?php 
	//On se place sur la table prestation et on utilise l'ID famille qui est une FK 
	$line = $controler->getUserEvent($idfamille); ?>
</div>

<div class="container-fluid" style="background-color: white">
	<!-- Si on ne trouve aucune prestation dans la table de l'utilsateur dans la bdd -->
	<!-- On affiche un message  -->
	<?php if($line == "Aucun event."): ?>
			<div class="alert alert-primary" role="alert" style="width: 250px">
  					Aucune evenement trouvé. 
					</div>
					<?php endif ?>
	<?php if($line != "Aucun event."): ?>
	<!--  Si on trouve des prestations dans la base alors on les affiche dans un tableau -->
  		...
  		<tbody>
  			<?php foreach ($line as $lines => $value): ?>
    			<tr>
      				...
      				<td><?php echo $value['heureevent']; ?></td>
      				<td><?php echo $value['tailleevent']; ?></td>
    			</tr>
    		<?php endforeach; ?>
		</tbody>
	</table>
